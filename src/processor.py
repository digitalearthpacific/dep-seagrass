...
class SeagrassProcessor:
    def __init__(self, model, probability_threshold: int = 60):
        self._model = model
        self._probability_threshold = 60

    def process(self, data: xr.Dataset) -> xr.Dataset:
        scaled_data = scale(data)
        texture = texture(data)
        combined_ds = xr.merge([scaled_data, texture])
...
def texture(data: xr.Dataset, window_size=9, levels=32) -> xr.Dataset:
    # Input
    max = data.blue.max().values
    min = data.blue.min().values
    # Scale to 0-LEVELS for GLCM
    img = (
        ((scaled.blue - min) / (max - min) * (levels - 1))
        .clip(0, LEVELS - 1)
